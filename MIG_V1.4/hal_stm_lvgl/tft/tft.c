...
//__attribute__((section("ExtFlashSection"), used)) static uintpixel_t my_fb[TFT_HOR_RES * TFT_VER_RES];

__attribute__((section("sdram1"), used)) static uintpixel_t my_fb[TFT_HOR_RES * TFT_VER_RES];
//__attribute__((section("sdram1"), used)) static uintpixel_t my_fb1[TFT_HOR_RES * TFT_VER_RES];
...
static void LCD_MspInit(void)
{
    ...
    __HAL_RCC_GPIOA_CLK_ENABLE();  // Enable clock for GPIOA (replace GPIOA with the correct port)

    GPIO_InitStruct.Pin = GPIO_PIN_5;  // Set the pin RA5
    ...
    HAL_GPIO_Init(GPIOA, &GPIO_InitStruct);  // Initialize the GPIO
...
static void LCD_LayerRgb565Init(uint32_t FB_Address)
{
    ...
    layer_cfg.FBStartAdress = (uint32_t)my_fb;
//    layer_cfg.FBStartAdress = 0xC0000000;
    /* Layer Init */
