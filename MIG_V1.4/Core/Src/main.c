...
void uart_code_for_dc_motor(void);
...
int duty_percent = 50;  ///duty cycle in percentage
...
//			uart_code_for_dc_motor();
			  if (HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_15) == GPIO_PIN_RESET)
				{
				  HAL_GPIO_WritePin(GPIOI, GPIO_PIN_1, GPIO_PIN_SET);  // Turn on LED
				  HAL_Delay(50);
				}
				else
				{
				  HAL_GPIO_WritePin(GPIOI, GPIO_PIN_1, GPIO_PIN_RESET);  // Turn off LED
				  HAL_Delay(50);
				}
	...
	void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
	{
		    ...
		    if (GPIO_Pin == GPIO_PIN_3)  // A (CLK) pin on PI1
		    {
		  ...
		  if (duty_percent < 10.0f && brake_started == 0 && brake_finished == 0)
		        {
		            ...
		            brake_started = 1;
		            sprintf(buffer, "Brake start time: %d ms\r\n", brake_start_time);
