...
lv_obj_t *canvas_list[4];
uint8_t current_canvas_index = 0;

static lv_style_t style_highlighted;
...
void encoder_read_cb(lv_indev_drv_t *drv, lv_indev_data_t *data)  // Fix return type
{
    static int16_t last_encoder = 0;
    int16_t encoder_val = __HAL_TIM_GET_COUNTER(&htim3);

    int16_t diff = encoder_val - last_encoder;
    if (diff > 10000) diff -= 0xFFFF;
    if (diff < -10000) diff += 0xFFFF;

    data->enc_diff = diff;
    data->state = HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_15) == GPIO_PIN_RESET ? LV_INDEV_STATE_PR : LV_INDEV_STATE_REL;

    last_encoder = encoder_val;
}

// ========== Highlight Logic ==========
void update_canvas_highlight(int dir)
{
    lv_obj_remove_style(canvas_list[current_canvas_index], &style_highlighted, 0);
    lv_obj_add_style(canvas_list[current_canvas_index], &style_normal, 0);

    current_canvas_index = (current_canvas_index + dir + 4) % 4;

    lv_obj_remove_style(canvas_list[current_canvas_index], &style_normal, 0);
    lv_obj_add_style(canvas_list[current_canvas_index], &style_highlighted, 0);
}



void check_encoder_change(void)
{
    static int16_t prev_encoder = 0;
    int16_t val = __HAL_TIM_GET_COUNTER(&htim3);
    int16_t diff = val - prev_encoder;

    if (diff > 1 || diff < -1)
    {
        update_canvas_highlight(diff > 0 ? 1 : -1);
        prev_encoder = val;
    }

    if (HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_15) == GPIO_PIN_RESET)
    {
        lv_event_send(canvas_list[current_canvas_index], LV_EVENT_CLICKED, NULL);
...
//
	...
	lv_style_init(&style_highlighted);
	lv_style_set_outline_width(&style_highlighted, 3);
	lv_style_set_outline_color(&style_highlighted, lv_color_make(255, 192, 203));  // RGB green
...
//    lv_obj_add_style(text_wire_size, &style, 0);  // Apply the style to all states
	...
	lv_style_set_outline_width(&style_normal, 0);

    lv_obj_add_style(canvas_list[0], &style_highlighted, 0);
    lv_obj_add_style(canvas_list[1], &style_normal, 0);
